...
public final class PasswordSpecification implements Parcelable {
    ...
    public Protobufs.PasswordSpecification toProtocolBuffer() {
        return Protobufs.PasswordSpecification.newBuilder()
                .setMinSize(minimumSize)
                .setMaxSize(maximumSize)
                .setAllowed(allowedChars)
                .addAllRequiredSets(convertRequiredCharSetsToProto())
                .build();
    }

    @Override
    public void writeToParcel(Parcel out, int flags) {
        byte[] encoded = toProtocolBuffer().toByteArray();
    ...
    private List<Protobufs.RequiredCharSet> convertRequiredCharSetsToProto() {
        List<Protobufs.RequiredCharSet> convertedRequiredCharSets = new ArrayList<>();

        for (RequiredCharSet set : requiredCharSets) {
            convertedRequiredCharSets.add(
                    Protobufs.RequiredCharSet.newBuilder()
                            .setChars(set.chars)
                            .setCount(set.count)
                            .build());
        }
